...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case baron:
        return playBaron(choice1, state);
...
{
    if (!validateAmbassadorChoices(choice1, choice2, state, handPos))
    {
        return -1;
    }
    ...
    for (i = 1; i < state->numPlayers; i++) // This is a bug because i should be initialized to 0 at the start of the FOR loop. Introduced in Assignment 2
    {
        if (i != currentPlayer)
        {
    ...
    for (j = 0; j < choice2; j++)
    {
        for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
            ...
            {
                discardCard(i, currentPlayer, state, 0); // This is a bug because the card isn't being trashed, only discarded. The trashFlag variable should be set to 1, not 0. Introduced in Assignment 2
                break;
            }
        }
    }
...
{
    if (!validateMineChoices(choice1, choice2, state))
    {
        return -1;
    }
...
int playMinion(int choice1, int choice2, struct gameState *state, int handPos)
{
    ...
    {
        discardHandAndRedraw(state, handPos, 4, currentPlayer);

        //other players discard hand and redraw if hand size > 4
        for (i = 0; i < state->numPlayers; i++)
        {
            if (state->handCount[i] >= 4) // This is a bug because it should be "> 4" not ">= 4". Introduced for Assignment 2. This is also a bug because it doesn't check which player we're discarding and redrawing for. This will mean that once the loop reaches the current player, that player will incorrectly discard their hand (we know for certain they will have exactly four cards because they just redrew them a few lines above) and redraw four cards.
            {
                discardHandAndRedraw(state, handPos, 4, i);
            }
        }
    }
...
int playTribute(struct gameState *state)
{
    ...
    int i;
    for (i = 0; i <= 2; i ++)  // This looks like a bug. The tributeRevealedCards array only has two elements but this loop will try to access a third element when i=2
    {
        if (isTreasureCard(tributeRevealedCards[i]))
        {
            state->coins += 2;
        }
        else if (isVictoryCard(tributeRevealedCards[i]))
        {
            drawCard(currentPlayer, state);
            drawCard(currentPlayer, state);
        }
...
int validateMineChoices(int choice1, int choice2, struct gameState *state)
{
    ...
    if (state->hand[currentPlayer][choice1] < copper || state->hand[currentPlayer][choice1] > silver) // This is a bug. The second condition should be checking "> gold" not "> silver". Introduced in Assignment 2.
    {
        return 0;
    }
...
void gainEstateForBaron(struct gameState *state)
{
    ...
    if (supplyCount(estate, state) > 0)
    {
        ...
        if (supplyCount(estate, state) == -1) // This is a bug because the function won't properly check if the game is over if the number of Estate cards goes to zero as a result of this player's turn. Introduced for Assignment 2.
        {
            isGameOver(state);
        }
    }
