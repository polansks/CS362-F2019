...
int cardCount(struct gameState *s, int player, int cardType, int search);
...
int main(int argc, char** argv)
{
	...
	for (i = 0; i < NUM_TRIALS; i++)
	{
		...
		int returnValue = baronEffect(choice1, &stateAfter, whoseTurn(&stateAfter));
...
void checkAssertions(int trialNumber, BOOL discardedEstate, int returnValue, struct gameState *before, struct gameState *after)
{
	...
	ASSERT(cardCount(after, player, baron, HAND) == cardCount(before, player, baron, HAND) - 1); // 1 fewer baron
	ASSERT(after->numBuys == before->numBuys + 1);

	if (discardedEstate && cardCount(before, player, estate, HAND) > 0)
	{
		ASSERT(after->coins == (before->coins + 4));// 4 more coins
		ASSERT(cardCount(after, player, estate, HAND) == cardCount(before, player, estate, HAND) - 1); // 1 fewer estate
	...
	else
	{
		...
		{
			ASSERT(cardCount(after, player, estate, DISCARD) == cardCount(before, player, estate, DISCARD) + 1); // 1 more estate
...
int cardCount(struct gameState *s, int player, int cardType, int search)
{
	...
	{
	case HAND:
		searchArray = s->hand[player];
		searchCount = s->handCount;
		break;

	case DISCARD:
		searchArray = s->discard[player];
		searchCount = s->discardCount;
		break;

	case DECK:
		searchArray = s->deck[player];
		searchCount = s->deckCount;
		break;

	}

	int i, count = 0;
	for (i = 0; i < searchCount[player]; i++)
	{
		if (searchArray[i] == cardType)
		{
			count++;
		}
	}
